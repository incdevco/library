...
class Inclusive_View_Helper_Picker extends Zend_View_Helper_FormText
{
	
	protected $_javascriptFormat = '%c/%e/%Y %h:%i:%s %p';
	
	protected $_phpFormat = 'n/j/Y h:i:s a';
...
HEREDOC;
			...
			$this->view->JQuery()->addOnload('$("input.anytime").AnyTime_picker({format:"'.$this->getJavascriptFormat().'"}); $("input.anytime").each(function(){ var clearButton = $("<input type=\"button\" value=\"Clear\" />"); clearButton.click(function(){ $(this).prev("input.anytime").val("").change(); }); $(this).after(clearButton); });');
